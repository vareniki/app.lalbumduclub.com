...
( function () {
	...
	document.getElementById( 'btn-crear-usuario' )?.addEventListener( 'click', async () => {
		...
		if ( ! username || ! email || ! password ) {
			errorEl.textContent = 'L\'identifiant, l\'email et le mot de passe sont obligatoires.';
		...
		try {
			...
			if ( ! res.success ) {
				errorEl.textContent = res.data || 'Erreur lors de la création de l\'utilisateur.';
				errorEl.classList.remove( 'tw:hidden' );
				return;
			}
	...
	document.getElementById( 'usuarios-lista' )?.addEventListener( 'click', async ( e ) => {
		...
		if ( e.target.closest( '.btn-delete-usuario' ) ) {
			...
			if ( ! confirm( `Supprimer l'utilisateur "${ nombre }" ? Cette action est irréversible.` ) ) return;
	...
	async function saveUser( item ) {
		...
		try {
			...
			if ( ! res.success ) {
				errorEl.textContent = res.data || 'Erreur lors de l\'enregistrement.';
				errorEl.classList.remove( 'tw:hidden' );
				return;
			}
			...
			const clubLabel = res.data.club_slug ? clubNombre( res.data.club_slug ) : 'Non assigné';
			if ( clubEl ) {
	...
	async function deleteUser( btn, userId ) {
		...
		try {
			...
			if ( lista && ! lista.querySelector( '.usuario-item' ) ) {
				...
				p.textContent = 'Aucun utilisateur avec le rôle Club pour l\'instant.';
				lista.appendChild( p );
			}
	...
	function appendUserRow( user ) {
		...
		if ( ! lista.querySelector( '.grid[class*="sm:grid-cols-"]' ) ) {
			...
			header.innerHTML = '<span>Nom / Identifiant</span><span>Email</span><span>Club assigné</span><span></span>';
		...
		div.innerHTML = `
			...
			<div class="usuario-edit-panel tw:hidden tw:border-t tw:border-gray-100 tw:px-6 tw:py-4 tw:bg-gray-50">
				<div class="tw:grid tw:grid-cols-1 tw:sm:grid-cols-2 tw:gap-3">
					...
					<div>
						<label class="tw:block tw:text-xs tw:text-gray-500 tw:mb-1">Nouveau mot de passe <span class="tw:text-gray-400 tw:font-normal">(vide = ne pas changer)</span></label>
